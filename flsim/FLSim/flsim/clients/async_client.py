...
class AsyncClientDevice(DeviceState):
    ...
    def train_local_model(
        ...
        # Store current model
        if self.client.cfg.store_folder is not None:
            store_current_model(
                after_train_local.fl_get_module(),
                self.client.cfg.store_folder,
                self.client.name,
                self.client.global_round_num,
                self.client.cfg.num_bits,
                self.client.cfg.scaling_factor,
            )
        # 3. Compute delta
