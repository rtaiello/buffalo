...
from flsim.extra.util import store_current_model
...
class Client:
    ...
    def generate_local_update(
        ...
        # 4. Store the updated model
        if self.cfg.store_folder is not None:
            store_current_model(
                updated_model.fl_get_module(),
                self.cfg.store_folder,
                self.name,
                self.global_round_num,
                self.cfg.num_bits,
    ...
    def train(
        ...
        n_sgd = (
            self.cfg.n_sgd
            if self.cfg.n_sgd > 0
            else self.dataset.num_train_batches() * epochs
        )
        ...
        for i in range(n_sgd + 1):
            ...
            self.post_batch_train(model, sample_count, optimizer)
        ...
        total_samples = self.dataset.num_train_examples()
    ...
    def post_batch_train(self, model: IFLModel, sample_count: int, optimizer: Any):
...
class ClientConfig:
    ...
    epochs: int = 0  # Number of epochs for local training
    n_sgd: int = 0  # Number of SGD steps to take
    ...
    store_folder: Optional[str] = None  # folder to save the model
    num_bits: Optional[int] = None  # number of bits for quantization
    scaling_factor: Optional[int] = None  # scaling factor for quantization
