...
class TestSecAggRoundReducer(TestRoundReducerBase):
    def test_sec_agg_round_reducer(self) -> None:
        model = utils.SampleNet(utils.TwoFC())
        sec_agg_rr = instantiate(
            "flsim.reducers.sec_agg_round_reducer.SecAggRoundReducer",
            global_model=model,
            reduction_type=ReductionType.AVERAGE,
            num_users_per_round=1,
            total_number_of_users=1,
        )
        assertIsInstance(sec_agg_rr, RoundReducer)

    def test_sec_agg_round_reducer_multiprocess(self) -> None:
        model = utils.SampleNet(utils.TwoFC())
        sec_agg_rr = instantiate(
            "flsim.reducers.sec_agg_round_reducer.SecAggRoundReducer",
            global_model=model,
        ...
        results = run_reduction_test(sec_agg_rr, num_processes=2, num_models=4)
        value_expected = float(sum(i + 1 for i in range(4)) / 4)
        for r in results:
            assertAlmostEqual(r, value_expected, 5)
