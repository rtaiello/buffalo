...
4. Run the pre-processing script:
    - `/preprocess.sh --sf 1.0 -k 30 -s niid -t 'user' --tf 0.50 --spltseed 1`
    ...
    def __init__(
        ...
        for user_id, user_data in self.dataset["user_data"].items():
            if user_id in user_ids:
                ...
                count_user_data += len(user_data["x"])
                self.targets[user_id] = list(user_data["y"])
        self.mean_user_data = count_user_data / num_users
...
def build_data_provider(data_config):
    ...
    train_dataset = CelebaDataset(
        data_root="leaf/data/celeba/data/train/all_data_0_5_keep_5_train_5.json",
        image_root="leaf/data/celeba/data/raw/",
        transform=transform,
    )
    test_dataset = CelebaDataset(
        data_root="leaf/data/celeba/data/test/all_data_0_5_keep_5_test_5.json",
        transform=transform,
    ...
    return data_provider, train_dataset.mean_user_data
...
def main_worker(
    ...
    data_provider,avg_num_examples = build_data_provider(data_config)
    model = (
