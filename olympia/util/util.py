...
from nacl.public import EncryptedMessage, PublicKey
...
from .paillier.phe_utils import EncryptedNumberPaillier, PublicKeyPaillier
from .ss.integer_ss import IShare
from .ss.shamir_ss import Share
from .elgamal.teg_utils import PublicKeyEG, EncryptedPointEG
...
def generate_latency_matrix(sub_df):
    ...
    for i in range(sub_df.shape[0]):
        for j in range(sub_df.shape[0]):
            combined_latency = (
                sub_df["avg_lat_ms"].iloc[i] + sub_df["avg_lat_ms"].iloc[j]
            )
            distanceAB = geopy.distance.geodesic(
                (sub_df["lat"].iloc[i], sub_df["lon"].iloc[i]),
                (sub_df["lat"].iloc[j], sub_df["lon"].iloc[j]),
            ).km
            latencyAB = (
                0.015 * distanceAB
            ) + combined_latency  # 0.015 = speedOfLight distance according to https://blog.scaleway.com/understanding-network-latency/
    ...
    G = nx.from_pandas_edgelist(
        new_df.rename(columns={"latencyAB": "weight"}),
        source="A",
        target="B",
        edge_attr="weight",
    )
...
def get_msg_size(msg):
    if isinstance(msg, dict):
        ...
        return len(msg)
    elif isinstance(msg, (PublicKey, EncryptedMessage, VerifyKey)):
    ...
    elif isinstance(msg, EncryptedNumberPaillier):
        return msg.get_real_size() // 8
    elif isinstance(msg, EncryptedNumberJL):
        return msg.get_real_size() // 8
    elif isinstance(msg, PublicKeyPaillier):
        return msg.n.bit_length() // 8
    elif isinstance(msg, IShare):
        return msg.get_real_size() // 8
    elif isinstance(msg, PublicKeyEG):
        return msg.get_real_size() // 8
        ...
        return msg.nbytes
    elif isinstance(msg, EncryptedPointEG):
        return msg.get_real_size() // 8

    elif isinstance(msg, EccPoint):
        return (int(msg.x).bit_length() + int(msg.y).bit_length()) // 8
        ...
        return 8
    elif isinstance(msg, rlwe_sa.cc.python._shell_encryption.SymmetricRlweCiphertext):
        bit_size = msg.num_coeffs() * msg.log_modulus()
        return bit_size // 8

    raise RuntimeError(
        f"Failed to compute message size for unknown message of type {type(msg)}: {msg}"
    )
...
def build_stats(server, clients, kernel):
    avgClientComputationTime = ns_to_ms(
        np.mean([c.total_computation_time for c in clients])
    )
    ...
    k = clients[0].K if hasattr(clients[0], "K") else 0
    t = clients[0].T if hasattr(clients[0], "T") else 0
