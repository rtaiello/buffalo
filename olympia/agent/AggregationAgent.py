...
class AggregationClient(Agent):
    ...
    def receiveMessage(self, currentTime, msg):
        ...
        t2 = time.time()
        ct = int((t2 - t1) * 1e9)
        if self.rounds_to_skip and msg.body["round_number"] in self.rounds_to_skip:
            ct = 0
        if self.rounds_to_add and msg.body["round_number"] in self.rounds_to_add.keys():
            if self.params["dim"] in self.rounds_to_add[msg.body["round_number"]]:
                ct += self.rounds_to_add[msg.body["round_number"]][self.params["dim"]]
        ...
        if output_msg is not None:
            self.sendMessage(
                ...
                Message(
                    {
                        "round_number": msg.body["round_number"],
                        "sender": self.id,
                        "body": output_msg,
                    }
                ),
...
class AggregationServer(Agent):
    ...
    def next_round(self, round_number, messages):
        raise RuntimeError(
            "Aggregation servers need to override the `next_round` method"
        )
    ...
    def wakeup(self, currentTime):
        ...
        t1 = time.time()
        output_msgs = self.round(
            self.round_number, self.received_messages[self.round_number - 1]
        )
        t2 = time.time()
        ct = int((t2 - t1) * 1e9)
        if self.rounds_to_skip and self.round_number in self.rounds_to_skip:
            ct = 0
        self.total_computation_time += ct
        self.delay(ct)
    ...
    def receiveMessage(self, currentTime, msg):
        ...
        if msg.body["round_number"] == self.round_number:
            self.received_messages[self.round_number][msg.body["sender"]] = msg.body[
                "body"
            ]

        if self.next_round(
            self.round_number, self.received_messages[self.round_number]
        ):
            self.round_number += 1
...
class DropoutAggregationServer(AggregationServer):
    ...
    def __init__(self, id, name, type, client_ids, params, random_state=None):
        ...
        self.dropout_threshold = len(client_ids) - self.dropout_fraction * len(
            client_ids
        )
    ...
    def succeed(self, result):
        self.total_dropouts = len(self.clients) - len(
            self.received_messages[self.round_number - 1]
        )
        self.total = result
        self.failure = 0
