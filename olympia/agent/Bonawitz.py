    ...
    def round(self, round_number, message):
        ...
        if round_number == 1:  # generate keys
            ...
            self.sk_u_shares = shamir.share_array(
                util.bytes_to_field_array(self.sk_u._private_key, self.GF),
                n_range,
                len(n_range) // 2,
                self.GF,
            )
            ...
            enc_bu_share = {
                c: self.b_u_shares[c].encrypt(self.sk_u, pk)
                for c, pk in self.pks.items()
                if c in self.b_u_shares
            }

            enc_sku_share = {
                c: self.sk_u_shares[c].encrypt(self.sk_u, pk)
                for c, pk in self.pks.items()
            ...
            dec_bu_share = {
                c: s.decrypt(self.sk_u, self.pks[c])
