...
class BellClientAgent(AggregationClient):
    def round(self, round_number, message):
        ...
        if round_number == 1:  # generate keys
            ...
            self.neighbor_pks1 = {
                k: v
                for k, v in self.neighbor_pks1_with_dropouts.items()
                if v is not None
            }
            self.neighbor_pks2 = {
                k: v
                for k, v in self.neighbor_pks2_with_dropouts.items()
                if v is not None
            }
            ...
            self.sk_u_shares = shamir.share_array(
                util.bytes_to_field_array(self.sk1_u._private_key, self.GF),
                n_range,
                len(n_range) // 2,
                self.GF,
            )
            ...
            enc_bu_share = {
                c: self.b_u_shares[c].encrypt(self.sk2_u, pk)
                for c, pk in self.neighbor_pks2.items()
                if c in self.b_u_shares
            }

            enc_sku_share = {
                c: self.sk_u_shares[c].encrypt(self.sk2_u, pk)
                for c, pk in self.neighbor_pks2.items()
            ...
            client_value = self.GF(
                self.random_state.randint(low=0, high=100, size=self.params["dim"])
            )
            ...
            dec_bu_share = {
                c: s.decrypt(self.sk2_u, self.neighbor_pks2[c])
            ...
            bu_share = {
                k: (
                    dec_bu_share[k]
                    if k in non_dropout_list
                    else shamir.ArrayShare(
                        dec_bu_share[k].x,
                        np.array([0]),
                        dec_bu_share[k].T,
                        dec_bu_share[k].GF,
