...
class BaseConfig:
    def __init__(self, config_file):
        ...
        parser.add_argument("-c", "--config", help="Name of config file to execute")
        parser.add_argument(
            "-l",
            "--log_dir",
            default=None,
        ...
        parser.add_argument(
            "-v", "--verbose", action="store_true", help="Maximum verbosity!"
        )
        parser.add_argument(
            "--config_help",
            action="store_true",
        ...
        parser.add_argument(
            "--no_save", action="store_true", help="Dont save the results"
        )
        ...
        self.GF = galois.GF(15969681560540810653)
        self.default_computation_delay = (
            0  # 1000000000 * config['default_computation_delay']
        )
        # self.default_computation_delay = 1000000000 * config['default_computation_delay']
        self.result_filename = (
            f"results/{self.protocol_name}_"
        ...
        self.participant_configurations = itertools.product(
            self.num_clients, self.dims, self.num_trials
        )
    ...
    def run_config(self):
        ...
        for num_clients, dim, trial_num in self.participant_configurations:
            print("===" * 40)
            print(
                f"{self.protocol_name} Experiment {trial_num}: RUNNING FOR {num_clients} CLIENTS and {dim} Dimensions"
            )
            dim = int(float(dim))
            ...
            kernel = Kernel(
                f"{self.protocol_name} Kernel",
                random_state=np.random.RandomState(
                    seed=np.random.randint(low=0, high=(2**32), dtype=np.int64)
                ),
            )

            server_random_state = np.random.RandomState(
                seed=np.random.randint(low=0, high=(2**32), dtype=np.int64)
            )
            self.params["dim"] = dim
            ...
            server = self.server_class(
                0,
                f"{self.protocol_name} Service Agent 0",
            ...
            clients = []
            for i in range(a, b):
                self.params["random_state"] = np.random.RandomState(
                    seed=np.random.randint(low=0, high=(2**32), dtype=np.int64)
                )
                clients.append(
                    self.client_class(
                        i,
                        f"{self.protocol_name} Client Agent {i}",
                        f"{self.protocol_name}ClientAgent",
                        params=self.params,
                    )
                )
            ...
            agent_types.extend(
                [f"{self.protocol_name}ClientAgent" for _ in range(a, b)]
            )
            ...
            kernel.runner(
                agents=agents,
                startTime=self.midnight,
                stopTime=self.midnight + pd.Timedelta("100:00:00"),
            ...
            if not self.args.no_save:
                util.write_csv(
                    self.result_filename, stats, num_clients, dim, dropout_wait
                )
